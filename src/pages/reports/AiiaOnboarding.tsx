...
import { ArrowLeft, Check, Plus, Trash2, Upload, ChevronRight } from "lucide-react";
import { AIIA_SESSION_KEY } from "@/pages/reports/AiiaPortal";
...
const LS_KEY = "aiia_onboarding_v1";
const LS_SUBMITTED_KEY = "aiia_onboarding_submitted";

// ─── GOLD THEME ──────────────────────────────────────────────────────────────
const G = "#C4973C";
const G_HOVER = "#D4A84A";
const BG = "#03050A";
const CARD = "#0A0C12";
...
const RadioOpt = ({ name, value, checked, onChange, label }: { name: string; value: string; checked: boolean; onChange: () => void; label: string }) => (
  <label className="flex items-center gap-2.5 cursor-pointer group" onClick={onChange}>
    <div className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors cursor-pointer"
      style={{ borderColor: checked ? G : "#4B5563", background: checked ? G : "transparent" }}>
...
const CheckOpt = ({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) => (
  <label className="flex items-center gap-2.5 cursor-pointer group" onClick={onChange}>
    <div className="w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors cursor-pointer"
      style={{ borderColor: checked ? G : "#4B5563", background: checked ? G : "transparent" }}>
...
const FL = ({ children, hint }: { children: React.ReactNode; hint?: string }) => (
  <div className="mb-2">
    <p className="text-sm font-medium" style={{ color: "#E5E7EB" }}>{children}</p>
...
const SC = ({ number, title, children }: { number: string; title: string; children: React.ReactNode }) => (
  <div className="rounded-2xl p-6 space-y-5" style={{ background: CARD, border: `1px solid rgba(196,151,60,0.15)` }}>
    <div className="flex items-center gap-3 mb-4">
      <span className="text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0"
        style={{ background: "rgba(196,151,60,0.15)", color: G, border: `1px solid rgba(196,151,60,0.25)` }}>
        {number}
      </span>
...
const Hr = () => <div className="border-t my-1" style={{ borderColor: "rgba(196,151,60,0.08)" }} />;
...
const TI = ({ value, onChange, placeholder = "", type = "text" }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) => (
  <input type={type} value={value ?? ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
    ...
    style={{ ...inputStyle, caretColor: G }}
    onFocus={(e) => { e.target.style.borderColor = `rgba(196,151,60,0.5)`; }}
    onBlur={(e) => { e.target.style.borderColor = `rgba(196,151,60,0.15)`; }}
  />
);
...
const AiiaOnboarding = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (sessionStorage.getItem(AIIA_SESSION_KEY) !== "aiia") { navigate("/aiia", { replace: true }); return; }
    setAuthorized(true);
    ...
    const savedPw = localStorage.getItem("aiia_viewpw");
  ...
  const sq = (id: string, topic: string) => (
    <div className="mt-2 pt-4 border-t" style={{ borderColor: "rgba(196,151,60,0.08)" }}>
  ...
  // ── SUBMITTED ──────────────────────────────────────────────────────────────
  if (submitted) return (
    <div className="min-h-screen text-white flex flex-col items-center justify-center px-6 py-20" style={{ background: BG }}>
      <div className="w-16 h-16 rounded-full flex items-center justify-center mb-6"
        style={{ background: "rgba(196,151,60,0.1)", border: "1px solid rgba(196,151,60,0.25)" }}>
        <Check className="w-7 h-7" style={{ color: G }} />
      </div>
      <h1 className="text-2xl font-bold mb-3">Questionnaire Submitted</h1>
      <p className="text-center max-w-sm leading-relaxed mb-6" style={{ color: "#6B7280" }}>
        Thank you! Your answers have been saved. We'll review everything and be in touch within 24 hours.
      </p>
      {viewPassword && (
        <div className="rounded-2xl px-8 py-6 text-center max-w-sm w-full mb-8"
          style={{ background: CARD, border: "1px solid rgba(196,151,60,0.2)" }}>
          <p className="text-xs mb-2 uppercase tracking-widest" style={{ color: "#6B7280" }}>Your access code</p>
          ...
          <p className="text-xs leading-relaxed mb-4" style={{ color: "#6B7280" }}>
            Use this code to view your submitted answers at any time.
          </p>
          <a href="/aiia/results" className="inline-block text-xs underline underline-offset-2 transition-colors"
            style={{ color: G }}
            onMouseEnter={(e) => { (e.target as HTMLElement).style.color = G_HOVER; }}
            onMouseLeave={(e) => { (e.target as HTMLElement).style.color = G; }}>
            reports.aiiaco.com/results →
          </a>
        </div>
      )}
      <button onClick={() => navigate("/aiia")} className="flex items-center gap-2 text-sm transition-colors" style={{ color: "#6B7280" }}>
  ...
  const cellInput = "min-w-[90px] px-2 py-1.5 rounded text-xs outline-none transition-colors";
  const cellInputStyle = { ...inputStyle, fontSize: "0.75rem" };

  return (
    <div className="min-h-screen text-white flex flex-col" style={{ background: BG }}>
      ...
      <div className="sticky top-0 z-50 backdrop-blur-md" style={{ background: `rgba(3,5,10,0.95)`, borderBottom: "1px solid rgba(196,151,60,0.1)" }}>
        <div className="flex items-center justify-between px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => navigate("/aiia")} className="flex items-center gap-1.5 text-sm transition-colors flex-shrink-0" style={{ color: "#6B7280" }}>
              <ArrowLeft className="w-4 h-4" /><span className="hidden sm:inline" style={{ color: "#6B7280" }}>Portal</span>
            </button>
            <span className="hidden sm:inline" style={{ color: "#374151" }}>|</span>
            <span className="text-xs sm:text-sm truncate" style={{ color: "#6B7280" }}>AiiA — Production Onboarding</span>
          </div>
          <div className={`flex items-center gap-1.5 text-xs transition-opacity duration-300 ${showSaved ? "opacity-100" : "opacity-0"}`}>
            ...
            <span style={{ color: G }}>Saved {savedAt}</span>
          </div>
        </div>
        {/* Section tabs */}
          ...
          {SECTIONS.map((s) => (
            <button key={s.id} data-section={s.id} onClick={() => scrollToTab(s.id)}
              ...
              style={activeSection === s.id
                ? { background: G, color: "#03050A", fontWeight: 600 }
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-2">
            <img src="/aiia-logo.png" alt="AiiA" className="h-8 w-auto object-contain" />
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: G }}>PRODUCTION ONBOARDING</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Production Onboarding</h1>
          <p className="text-sm mt-1" style={{ color: "#6B7280" }}>Please complete all sections at your own pace — your answers save automatically.</p>
        </div>
          ...
          <SC number="02" title="Staff Roster">
            <div>
                      ...
                      <tr key={i} style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
                        ...
                        <td className="pr-2 py-2">
                          <select value={row.role} onChange={(e) => { const n=[...staffRows]; n[i]={...n[i],role:e.target.value}; setStaffRows(n); }}
                            className="w-full min-w-[120px] px-2 py-1.5 rounded text-xs outline-none"
                        ...
                        <td className="pr-2 py-2">
                          <input value={row.properties} onChange={(e) => { const n=[...staffRows]; n[i]={...n[i],properties:e.target.value}; setStaffRows(n); }}
                            className={`${cellInput} w-full min-w-[110px]`} style={cellInputStyle}
                            onFocus={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.5)"; }}
                            onBlur={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.15)"; }} />
                        </td>
              ...
              <button onClick={() => setStaffRows([...staffRows,{name:"",email:"",phone:"",role:"field_staff",properties:""}])}
                className="mt-3 flex items-center gap-1.5 text-xs transition-colors" style={{ color: G }}>
        ...
        {activeSection === "s03" && (
          <SC number="03" title="Task Types & Categories">
            <p className="text-xs -mt-2" style={{ color: "#6B7280" }}>Maintenance</p>
            ...
            <Hr />
            <p className="text-xs" style={{ color: "#6B7280" }}>Housekeeping</p>
            ...
            <Hr />
            <p className="text-xs" style={{ color: "#6B7280" }}>Inspections</p>
        ...
        {activeSection === "s04" && (
          <SC number="04" title="Task Status & Completion Rules">
            <div className="rounded-xl px-4 py-3 border text-xs" style={{ background: "rgba(196,151,60,0.04)", borderColor: "rgba(196,151,60,0.15)", color: "#9CA3AF" }}>
              Our status flow: <span className="text-white">new → assigned → in_progress → waiting_parts → blocked → completed → verified</span>
            </div>
              ...
              <div className="space-y-3">
                {["Urgent","High","Medium","Low"].map((lvl) => (
                  <div key={lvl} className="flex gap-3 items-start">
                    <span className="text-xs font-semibold mt-2.5 w-14 flex-shrink-0"
                      style={{ color: lvl==="Urgent"?"#F87171":lvl==="High"?"#FB923C":lvl==="Medium"?"#FCD34D":"#9CA3AF" }}>
                      {lvl}
                    </span>
          ...
          <SC number="08" title="Notifications">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {NOTIF_EVENTS.map((event) => (
                      <tr key={event} style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
                        <td className="py-2.5 pr-4" style={{ color: "#D1D5DB" }}>{event}</td>
                        {NOTIF_ROLES.map((role) => {
                          const key = `s08_notif_${event}_${role}`;
                          return (
                            <td key={role} className="text-center py-2.5 px-2">
                              <div onClick={() => set(key, !formData[key])}
                                className="w-4 h-4 rounded border-2 flex items-center justify-center mx-auto cursor-pointer transition-colors"
                                style={{ borderColor: formData[key] ? G : "#4B5563", background: formData[key] ? G : "transparent" }}>
            ...
            {formData.s08_quiet === "Yes" && (
              <div className="ml-6 -mt-2 flex items-center gap-3">
                ...
                <span className="flex-shrink-0" style={{ color: "#6B7280" }}>to</span>
        ...
        {activeSection === "s09" && (
          <SC number="09" title="Inspection Templates">
            <p className="text-xs -mt-2 mb-1" style={{ color: "#6B7280" }}>For each type, describe checklist sections, items needing photos, and items that should auto-create a maintenance task. You can also upload existing checklists in Deliverables.</p>
        ...
        {activeSection === "s10" && (
          <SC number="10" title="Branding & UX">
            ...
            <Hr />
            {rg("s10_colors", "Brand colors for the app?", ["Yes, use LRMB brand colors (we'll share them)","Use AiiACo default design","Open to suggestions"])}
            <Hr />
        ...
        {activeSection === "s12" && (
          <SC number="12" title="Production Deployment">
            <div>
              ...
              {formData.s12_domain === "custom" && (
                <div className="ml-6 space-y-3">
                  ...
                  <p className="text-xs" style={{ color: "#6B7280" }}>How will you connect the domain?</p>
            ...
            <Hr />
            {rg("s12_accounts", "Who manages user accounts (create / remove) in production?", ["We manage it ourselves","AiiACo manages it for us","Shared — we handle most, you help with setup"])}
            <Hr />
        ...
        {activeSection === "s13" && (
          <SC number="13" title="Baseline KPI Data">
            <p className="text-xs -mt-2 mb-1" style={{ color: "#6B7280" }}>We need this before deployment to measure the improvement we achieve. Estimates are fine.</p>
        ...
        {activeSection === "s14" && (
          <SC number="14" title="Open Questions">
            ...
            <div>
                ...
                {adminRows.map((row, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <input value={row.name} onChange={(e) => { const n=[...adminRows]; n[i]={...n[i],name:e.target.value}; setAdminRows(n); }} placeholder="Full name"
                      className="flex-1 px-3 py-2 rounded-lg text-sm outline-none transition-colors" style={inputStyle}
                      onFocus={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.5)"; }}
                      onBlur={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.15)"; }} />
              ...
              <button onClick={() => setAdminRows([...adminRows,{name:"",email:""}])} className="mt-2 flex items-center gap-1.5 text-xs transition-colors" style={{ color: G }}>
        ...
        {activeSection === "del" && (
          <div className="space-y-4">
            <div className="rounded-2xl p-6" style={{ background: CARD, border: "1px solid rgba(196,151,60,0.15)" }}>
              ...
              <p className="text-xs mt-1 mb-6" style={{ color: "#6B7280" }}>
                Check each item you have ready, attach the file, and add a Google Drive link or notes. Email all files to <span style={{ color: "#9CA3AF" }}>contact@aiiaco.com</span>.
              </p>
                ...
                {DELIVERABLES.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="rounded-xl border p-4 transition-colors"
                      style={{ borderColor: formData[readyKey] ? "rgba(196,151,60,0.4)" : "rgba(196,151,60,0.12)", background: formData[readyKey] ? "rgba(196,151,60,0.05)" : "rgba(255,255,255,0.01)" }}>
                      <div className="flex items-start gap-3">
                        <div onClick={() => set(readyKey, !formData[readyKey])}
                          ...
                          style={{ borderColor: formData[readyKey] ? G : "#4B5563", background: formData[readyKey] ? G : "transparent" }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs mt-0.5" style={{ color: "#6B7280" }}>{item.format}</p>
                          <div className="mt-3 space-y-2">
                            <label className="flex items-center gap-2 text-xs cursor-pointer px-3 py-2 rounded-lg border transition-colors"
                              style={filePaths[item.key]
                                ? { borderColor:"rgba(196,151,60,0.4)", background:"rgba(196,151,60,0.05)", color: G }
                                : fileNames[item.key]
                                ? { borderColor:"rgba(196,151,60,0.2)", background:"rgba(196,151,60,0.03)", color: G }
                                : { borderColor:"rgba(196,151,60,0.1)", color:"#6B7280" }
                              }>
                              {fileUploading[item.key]
                                ? <div className="w-3.5 h-3.5 border-2 rounded-full animate-spin flex-shrink-0" style={{ borderColor:"rgba(255,255,255,0.2)", borderTopColor: G }} />
                                : filePaths[item.key]
                              ...
                              {filePaths[item.key] && <span className="ml-auto text-[10px] flex-shrink-0" style={{ color: G }}>Uploaded</span>}
                            ...
                            <input value={formData[notesKey] ?? ""} onChange={(e) => set(notesKey, e.target.value)} placeholder="Notes or Google Drive link..."
                              className="w-full px-3 py-2 rounded-lg text-xs outline-none transition-colors" style={{ ...inputStyle, fontSize: "0.75rem" }}
                              onFocus={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.5)"; }}
                              onBlur={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.15)"; }} />
                          </div>
                        </div>
            ...
            {/* Submit */}
            <div className="rounded-2xl p-6 text-center" style={{ background: CARD, border: "1px solid rgba(196,151,60,0.15)" }}>
              <h3 className="text-white font-semibold mb-2">Ready to submit?</h3>
              <p className="text-sm mb-6 leading-relaxed" style={{ color: "#6B7280" }}>
                Once you submit, your answers will be saved and we'll begin reviewing within 24 hours. You can still return and update your answers after submitting.
              </p>
              <button
                onClick={async () => {
                  setSubmitting(true);
                  ...
                  if (error) {
                    setSubmitError("Something went wrong. Please try again or email your answers to contact@aiiaco.com.");
                    setSubmitting(false);
                    return;
                  }
                  localStorage.setItem(LS_SUBMITTED_KEY, "true");
                  localStorage.setItem("aiia_viewpw", viewPw);
                  setViewPassword(viewPw);
                  setSubmitted(true);
                }}
                disabled={submitting}
                className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-semibold text-sm transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
                style={{ background: G, color: "#03050A" }}
                onMouseEnter={(e) => { if (!submitting) (e.currentTarget as HTMLElement).style.background = G_HOVER; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = G; }}
              >
                {submitting
                  ? <><div className="w-4 h-4 border-2 rounded-full animate-spin" style={{ borderColor: "rgba(3,5,10,0.3)", borderTopColor: "#03050A" }} /> Submitting...</>
                  : <><Check className="w-4 h-4" /> Submit Questionnaire</>
                }
              </button>
              {submitError && <p className="text-red-400 text-xs mt-3">{submitError}</p>}
              <p className="text-xs mt-4" style={{ color: "#374151" }}>Your answers are auto-saved — you can close this tab and return later.</p>
            </div>
          </div>
        )}
        ...
        {activeSection !== "del" && (
          <div className="flex justify-end pt-2">
            <button onClick={nextSection} className="flex items-center gap-2 text-sm transition-colors px-4 py-2 rounded-xl" style={{ color: "#6B7280" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#FFFFFF"; (e.currentTarget as HTMLElement).style.background = "rgba(196,151,60,0.08)"; }}
        ...
        <p className="text-xs text-center mt-6 pb-4" style={{ color: "#1F2937" }}>
          AiiACo · Answers are saved locally and only visible to you and your team.
        </p>
      </main>
    </div>
  );
