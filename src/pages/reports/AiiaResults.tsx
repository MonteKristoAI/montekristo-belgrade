...
const Ans = ({ label, value }: { label: string; value: any }) => {
  ...
  return (
    <div className="py-2.5 last:border-0" style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
      <p className="text-[11px] mb-0.5" style={{ color: "#6B7280" }}>{label}</p>
      <p className="text-sm whitespace-pre-wrap leading-relaxed" style={{ color: "#E5E7EB" }}>{formatValue(value)}</p>
    </div>
  );
};

const Sec = ({ number, title, children }: { number: string; title: string; children: React.ReactNode }) => (
  <div className="rounded-2xl p-6 space-y-0.5" style={{ background: CARD, border: "1px solid rgba(196,151,60,0.15)" }}>
...
const AiiaResults = () => {
  ...
  if (!sub) {
    return (
      <div className="min-h-screen text-white flex flex-col items-center justify-center px-6" style={{ background: BG }}>
        <div className="w-full max-w-sm">
          <div className="flex items-center gap-2 mb-8 justify-center">
            <img src="/aiia-logo.png" alt="AiiA" className="h-10 w-auto object-contain" />
            <span className="text-sm font-semibold" style={{ color: "#9CA3AF" }}>Results</span>
          </div>
          <div className="rounded-2xl p-8" style={{ background: CARD, border: "1px solid rgba(196,151,60,0.2)" }}>
            ...
            <p className="text-sm text-center mb-6" style={{ color: "#6B7280" }}>Enter your access code to view submitted answers.</p>
            <input
              type="text"
              value={pw}
              ...
              maxLength={9}
              className="w-full px-4 py-3 rounded-xl text-white text-center text-lg font-mono tracking-widest outline-none transition-colors mb-3"
              style={{ background: "rgba(255,255,255,0.05)", border: `1px solid rgba(196,151,60,0.2)`, caretColor: G }}
              onFocus={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.5)"; }}
              onBlur={(e) => { e.target.style.borderColor = "rgba(196,151,60,0.2)"; }}
            />
            ...
            <button
              ...
              onMouseEnter={(e) => { if (!loading) (e.currentTarget as HTMLElement).style.background = "#D4A84A"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = G; }}
            >
              {loading ? "Checking..." : "View Results"}
            </button>
          </div>
          <button onClick={() => navigate(-1)} className="mt-6 flex items-center gap-1.5 text-sm transition-colors mx-auto" style={{ color: "#6B7280" }}>
  ...
  const notifMatrix = () => {
    ...
    return (
      <div className="py-2.5" style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
        <p className="text-[11px] mb-2" style={{ color: "#6B7280" }}>Notification matrix</p>
        <div className="space-y-1.5">
          {rows.map(({ event, roles }) => (
            <div key={event} className="flex gap-3 text-xs">
              <span className="w-40 flex-shrink-0" style={{ color: "#6B7280" }}>{event}</span>
              <span style={{ color: "#E5E7EB" }}>{roles.join(", ")}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="sticky top-0 z-50 backdrop-blur-md" style={{ background: "rgba(3,5,10,0.95)", borderBottom: "1px solid rgba(196,151,60,0.1)" }}>
        <div className="px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm transition-colors" style={{ color: "#6B7280" }}>
              <ArrowLeft className="w-4 h-4" /><span className="hidden sm:inline">Back</span>
            </button>
            <span className="hidden sm:inline" style={{ color: "#374151" }}>|</span>
            <span className="text-xs sm:text-sm" style={{ color: "#6B7280" }}>AiiA — Onboarding Results</span>
          </div>
          <span className="text-xs" style={{ color: "#374151" }}>Submitted {submittedAt}</span>
        </div>
      </div>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-2">
            <img src="/aiia-logo.png" alt="AiiA" className="h-7 w-auto object-contain" />
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: G }}>ONBOARDING</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Submitted Questionnaire</h1>
          <p className="text-sm mt-1" style={{ color: "#6B7280" }}>Read-only view of submitted onboarding answers.</p>
        </div>
        ...
        <Sec number="02" title="Staff Roster">
          {(staffRows ?? []).some((r: any) => r.name) && (
            <div className="pb-3 mb-1" style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
              <p className="text-[11px] mb-2" style={{ color: "#6B7280" }}>Staff members</p>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {(staffRows ?? []).filter((r: any) => r.name).map((r: any, i: number) => (
                      <tr key={i} style={{ borderBottom: "1px solid rgba(196,151,60,0.05)", color: "#D1D5DB" }}>
        ...
        <Sec number="09" title="Inspection Templates">
          {["Move-in", "Move-out", "Periodic", "Damage"].map((type) => {
            ...
            return (
              <div key={type} className="pb-3 mb-1" style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
                <p className="text-[11px] font-semibold mb-1" style={{ color: "#9CA3AF" }}>{type} Inspection</p>
        ...
        <Sec number="14" title="Open Questions">
          ...
          {(adminRows ?? []).some((r: any) => r.name) && (
            <div className="py-2.5" style={{ borderBottom: "1px solid rgba(196,151,60,0.06)" }}>
              <p className="text-[11px] mb-1" style={{ color: "#6B7280" }}>Admin users</p>
              {(adminRows ?? []).filter((r: any) => r.name).map((r: any, i: number) => (
                <p key={i} className="text-sm" style={{ color: "#E5E7EB" }}>{r.name}{r.email ? ` · ${r.email}` : ""}</p>
              ))}
            </div>
          )}
        ...
        <p className="text-[11px] text-center pb-4" style={{ color: "#374151" }}>
          AiiACo · Read-only view · Contact contact@aiiaco.com to make changes.
        </p>
      </main>
    </div>
  );
